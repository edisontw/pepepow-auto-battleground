...
const CELL_EFFECT_TYPES = new Set<BattleEvent["type"]>(["damage", "critical", "heal", "shield", "stun", "control", "death"]);
...
function visibleCellEvents(events: BattleEvent[]) {
  return events.filter((entry) => CELL_EFFECT_TYPES.has(entry.type));
}

function impactClass(events: BattleEvent[]) {
  const type = events.at(-1)?.type;
  if (type === "heal") return "impact impact-heal";
  if (type === "shield") return "impact impact-shield";
  if (type === "stun" || type === "control") return "impact impact-control";
  return events.length ? "impact impact-damage" : "";
}
...
export default function Game() {
  ...
  const featuredCast = combatFrame ? [...combatFrame.events].reverse().find((entry) => entry.type === "cast" && entry.skillId) : undefined;
  const featuredCaster = featuredCast && combatFrame ? combatFrame.units.find((entry) => entry.uid === featuredCast.sourceUid) : undefined;
  const featuredSkill = featuredCast?.skillId ? UNIT_MAP[featuredCast.skillId] : undefined;
  ...
  if (!active) return (
    <main className="landing-shell">
      ...
      <section className="landing-card">
        <div className="brand-lockup"><img src="/pepepow-symbol.png" alt="PEPEPOW symbol" width="58" height="58" fetchPriority="high" /><div><span>PEPEPOW</span><small>READABILITY POLISH · v0.9.4</small></div></div>
        ...
        <section className="arena-panel">
          ...
          <div className={`battle-board ${phase === "battle" ? "in-combat" : ""}`}>
            ...
            {Array.from({ length: BOARD_COLS * BOARD_ROWS }, (_, position) => {
              ...
              const cellEvents = visibleCellEvents(combatFrame?.events.filter((event) => event.to === position) ?? []);
              return <div key={position} role="gridcell" tabIndex={position >= DEPLOY_START ? 0 : -1} data-drop-target={position >= DEPLOY_START && phase === "planning" ? `board:${position}` : undefined} className={`board-cell ${position >= DEPLOY_START ? "friendly" : "enemy"} ${(selectedUid || draggedUid) && position >= DEPLOY_START ? "available" : ""} ${dropTarget === `board:${position}` ? "drop-current" : ""} ${impactClass(cellEvents)}`} onClick={() => deploySelected(position)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") deploySelected(position); }} aria-label={`Board cell ${position + 1}${unit ? `, ${UNIT_MAP[unit.unitId].name}` : ", empty"}`}>
            ...
            {phase === "battle" && featuredSkill && featuredCaster && <div key={featuredCast!.id} className={`skill-banner team-${featuredCaster.team}`} aria-hidden="true"><small>ABILITY</small><span>{featuredCaster.team === "player" ? "ALLY" : "ENEMY"} · {featuredSkill.name}</span><b>{featuredSkill.skill}</b></div>}
...
function CellEffects({ events, units }: { events: BattleEvent[]; units: BattleFrame["units"] }) {
  return <div className="cell-effects" aria-hidden="true">{visibleCellEvents(events).slice(-4).map((entry, index) => {
