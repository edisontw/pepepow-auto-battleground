...
function Guide({ onClose }: { onClose: () => void }) {
  return <div className="modal-backdrop" onClick={onClose}><section className="modal guide-modal" role="dialog" aria-modal="true" aria-label="How to play" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose}>×</button><p className="eyebrow">FIELD MANUAL</p><h2>Build the crew. Make every round count.</h2><div className="guide-grid"><article><span>01</span><b>Recruit & fuse</b><p>Three identical copies fuse into ★★; three ★★ become ★★★. A full Bench still accepts a recruit if that purchase immediately fuses and leaves no more than eight units. Extra fusion equipment returns to inventory.</p></article><article><span>02</span><b>Move & inspect</b><p>Tap once to select. Drag between Bench and Board, between Board tiles, or along the Bench. Hold still for 450 ms—or right-click—to open Unit Info. Only a final drop explicitly inside the red Sell zone can sell a unit.</p></article><article><span>03</span><b>Targeting & Taunt</b><p>Melee units lock the nearest reachable enemy. Ranged units first keep a valid enemy already in range. Assassins start by prioritizing the enemy backline. Targets remain sticky; Guardian Taunt temporarily overrides them.</p></article><article><span>04</span><b>Gold & income</b><p>Each round pays 5 base Gold. Keep 10/20/30/40/50 Gold for +1 to +5 Interest. PvP win or loss streaks pay +1 at 2, +2 at 3, and +3 at 5; neutral battles preserve the streak.</p></article><article><span>05</span><b>XP, Level & Shop</b><p>Every completed battle grants +1 XP for rounds 1–2, +2 for 3–4, then rises every two rounds to a maximum +8. Training costs 4 Gold for +4 XP. Level sets unit cap and the exact Shop odds shown above the cards.</p></article><article><span>06</span><b>AI commanders</b><p>Each commander uses a real economy, Shop, roster and formation. Personality changes priorities; Easy, Normal and Hard change decision quality without hidden Gold, stats or odds.</p></article><article><span>07</span><b>Replay & analyze</b><p>Post-battle and Battle Archive show Top 5 Damage, Healing and Damage Taken. Replay v{REPLAY_VERSION} stores a deterministic seed for reproducible combat.</p></article><article><span>08</span><b>Performance & privacy</b><p>Rendering adapts between lighter and richer 30/60 FPS profiles while simulation stays authoritative. Anonymous diagnostics never include a wallet or personal identity.</p></article></div><button className="primary-action" onClick={onClose}>ENTER BATTLEGROUND</button></section></div>;
}

function targetingText(unitId: string) {
  const def = UNIT_MAP[unitId];
  return def.traits[1] === "Assassin" ? "Battle Start: prioritizes the enemy backline, then keeps that target until invalid." : def.range > 1 ? "Targeting: an enemy already in range; otherwise nearest reachable enemy. Target stays locked until invalid." : "Targeting: nearest reachable enemy. Target stays locked until invalid.";
}
